...
export default class AdminPanel extends Component {
  ...
  getWorkHours = (clinic) => {
    const dateList = [];
    Object.keys(clinic.availability).forEach((day) => {
      let hours = clinic.availability[day];
      if (hours.length) {
  ...
  getAppointments = (clinic) => {
    fetch(env.api + "/appointments/clinic/" + clinic._id.toString())
      .then((res) => res.json())
      .then((data) => {
  ...
  loadTimeSlots = (date) => {
    ...
    if (hours) {
      let startTime = new Date(
        date.toLocaleDateString("en-US") + " " + hours[0]
      );
  ...
  filterOutExistedAppointment = (date, timeSlotArray) => {
    ...
    this.state.existedAppointments.forEach((appointment) => {
      if (
        ...
        date.getTime() === new Date(appointment.date).getTime()
      ) {
        existedAppointmentTime.push(appointment.time);
      }
    });
    const filteredArray = timeSlotArray.filter(
      (value) => !existedAppointmentTime.includes(value)
    );
  ...
  onSubmit = () => {
    Axios({
      method: "POST",
      data: {
        clinicID: this.state.clinicSelected._id,
        clinicName: this.state.clinicSelected.name,
        patientID: this.state.userSelected._id,
        patientName:
          this.state.userSelected.firstName +
          " " +
          this.state.userSelected.lastName,
      ...
      <React.Fragment>
        ...
        <div className="contact-form-wraper">
          <div className="container new-container">
            <h3>Schedule an Appointment for Patient</h3>
            <div>
              ...
              <div className="col-lg-6 col-md-6 col-12 date-field">
                ...
                <Select
                  className="col-12"
                  classNamePrefix="select"
                  defaultValue={this.state.clinicSelected}
                  isSearchable={true}
            ...
            {this.state.userSelected && this.state.clinicSelected && (
              <div>
                <div className="col-lg-6 col-md-6 col-12 date-field">
                  <label>Date: </label>
                  <DatePicker
                ...
                {this.state.createAppointmentSuccess && (
                  <span className="error-msg">Appointment created</span>
                )}
              </div>
            )}
