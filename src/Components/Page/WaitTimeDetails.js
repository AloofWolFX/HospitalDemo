...
export default class WaitTimeDetails extends Component {
  ...
  loadDateFormat = () => {
    ...
    const appointmentDateFormatTemp = `${
      appointmentDate.getMonth() + 1
    }/${appointmentDate.getDate()}/${appointmentDate.getFullYear()}`;
    const currentDateFormatTemp = `${
      currentDate.getMonth() + 1
    }/${currentDate.getDate()}/${currentDate.getFullYear()}`;
    const currentTimeFormatTemp =
    ...
    this.setState(
      ...
      this.checkIfOneHourBeforeAppointment(
        appointmentDateFormatTemp,
        this.state.appointment.time,
        currentDateFormatTemp,
        currentTimeFormatTemp
      )
    );
  };

  checkIfOneHourBeforeAppointment = (
    appointmentDate,
    appointmentTime,
    currentDate,
    currentTime
    ...
    if (diff <= 60 && diff >= 0) {
      // within 1 hour of appointment time
      this.setState({
  ...
  countWaitInQueue = (appointments) => {
    ...
    const currentAt = new Date(
      this.state.appointmentDateFormat + " " + this.state.appointment.time
    );

    appointments.forEach((appointment) => {
      let appointmentAt = new Date(
        new Date(appointment.date).toISOString().replace(/T.*$/, "") +
          "T" +
          appointment.time
      );
      ...
      if (
        ...
        diff < 120 &&
        diff >= 0 &&
        appointment.patientID !== this.state.userLoggedIn._id
      ) {
        // get appointment count within next 2 hour
        waitCount++;
      }
    });
        ...
        <div className="user-form-wrapper">
          <div className="container">
            <div className="user-form-one">
              <h1>Virtual Queue</h1>
              <h2>
                Current time: {this.state.currentDateFormat}{" "}
                {this.state.currentTimeFormat}
              </h2>
              <h2>
                Your appointment time: {this.state.appointmentDateFormat}{" "}
                {this.state.appointment.time}
              </h2>
              <h2>
                Other patients waiting in queue: {this.state.waitCountInQueue}
              </h2>
              <h2>Estimated wait time: {this.state.waitTimeFormat}</h2>
              <br />
              <h3>Note: </h3>
              <h3>
                "Other patients waiting in queue" refers to how many
                appointments have been scheduled within 2-hour of your
                appointment time.
              </h3>
              <h3>
                Each appointment will increment the Estimated wait time by 30
                minutes as default.
              </h3>

              {this.state.ifOneHourBeforeAppointment &&
                this.state.checkInSuccess !== true &&
                this.state.appointment.ifCheckedIn !== true && (
                  <button className="contact-submit-btn" onClick={this.checkIn}>
                    Check in
                  </button>
                )}
